...
BEGIN
  ...
  IF v_action = 'create_reservation' THEN
    ...
    WHERE NULLIF(p_operation->>'table_id', '') IS NULL
       OR (c.allocation_type = 'table' AND c.allocation_id = (p_operation->>'table_id')::UUID)
    LIMIT 1;
  ...
  ELSE
    ...
    ELSIF v_action = 'seat_reservation' THEN
      IF v_reservation.status IN ('completed', 'cancelled', 'no_show') THEN RAISE EXCEPTION 'invalid_reservation_status'; END IF;
      IF v_reservation.table_id IS NULL AND v_reservation.group_id IS NULL THEN RAISE EXCEPTION 'reservation_has_no_table'; END IF;
      UPDATE reservations SET status = 'seated' WHERE id = v_id RETURNING * INTO v_reservation;
      IF v_reservation.table_id IS NOT NULL THEN
        UPDATE tables SET status = 'occupied', occupied_since = now(), current_reservation_id = v_id
        WHERE id = v_reservation.table_id
          AND room_id IN (SELECT id FROM rooms WHERE tenant_id = v_tenant);
      ELSE
        UPDATE tables SET status = 'occupied', occupied_since = now(), current_reservation_id = v_id
        WHERE id IN (SELECT table_id FROM table_group_members WHERE group_id = v_reservation.group_id)
          AND room_id IN (SELECT id FROM rooms WHERE tenant_id = v_tenant);
      END IF;
